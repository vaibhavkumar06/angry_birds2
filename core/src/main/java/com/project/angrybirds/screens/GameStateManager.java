...
public class GameStateManager {
    ...
    public static int loadSavedLevel() {
        HashMap<String, Object> gameState = loadGame();
        if (gameState != null && gameState.containsKey("level")) {
            return (int) gameState.get("level");  // Retrieve the saved level
        }
        return 0;  // Default value indicating no saved level
    }
}
